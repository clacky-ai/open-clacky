...
WS.onEvent(ev => {
  ...
  switch (ev.type) {
    ...
    case "tool_call":
      if (ev.session_id !== Sessions.activeId) break;
      if (window.ClackyUiPolicy?.chatEnabled("show_tool_trace") === false) break;
      Sessions.clearProgress();
    ...
    case "token_usage":
      if (ev.session_id !== Sessions.activeId) break;
      if (window.ClackyUiPolicy?.chatEnabled("show_token_usage") === false) break;
    ...
    case "complete":
      ...
      {
        const showCost = window.ClackyUiPolicy?.chatEnabled("show_cost") !== false;
        let mainLine;
        if (showCost) {
          ...
          const costDisplay = (!costSource || costSource === "estimated")
            ? "N/A"
            : `$${(ev.cost || 0).toFixed(4)}`;
          mainLine = I18n.t("chat.done", { n: ev.iterations, cost: costDisplay });
        } else {
          mainLine = I18n.t("chat.done.noCost", { n: ev.iterations });
        }
        if (typeof ev.duration === "number" && ev.duration > 0) {
          mainLine += I18n.t("chat.done.duration", { duration: ev.duration.toFixed(1) });
        }
        let cacheLine = null;
        if (window.ClackyUiPolicy?.chatEnabled("show_token_usage") !== false) {
    ...
    case "info":
      if (
        window.ClackyUiPolicy?.chatEnabled("show_tool_trace") === false &&
        String(ev.message || "").startsWith("Injected skill content")
      ) break;
