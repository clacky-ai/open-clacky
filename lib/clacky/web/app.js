...
WS.onEvent(ev => {
  switch (ev.type) {
    ...
    case "tool_call":
      if (ev.session_id !== Sessions.activeId) break;
      Sessions.clearProgress();
      Sessions.appendToolCall(ev.name, ev.args);
      break;
...
// Check onboard status first. If onboarding is needed, Onboard.check()
// shows the onboard panel and takes over. Normal boot (WS + sessions) is
// deferred until onboarding completes (via Onboard._bootUI()).
(async () => {
