  ...
  switch (ev.type) {
    ...
    case "progress":
      ...
      if (ev.status === "start") Sessions.showProgress(ev.message || I18n.t("chat.thinking"));
        ...
        Sessions.clearShellOutput();
        Sessions.clearProgress();
      }
      break;

    case "shell_output":
      if (ev.session_id && ev.session_id !== Sessions.activeId) break;
      Sessions.appendShellOutput(ev.text || "");
      break;
...
const SkillAC = (() => {
  ...
  function _getSlashQuery(value) {
    // Support Chinese input: ／ (full-width slash) and 、 (Chinese comma/dunhao)
  ...
  async function _render(query) {
    ...
    // Filter by query and optionally hide system skills
    let filtered = all.filter(s => s.name.toLowerCase().startsWith(query));
    if (!_showSystemSkills) {
      filtered = filtered.filter(s => s.source_type !== "default");
    }
    _items = filtered;
