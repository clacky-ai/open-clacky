  ...
  function _apply(view, params = {}) {
    ...
    switch (view) {
      ...
      case "session": {
        ...
        // Fallback: if no 'subscribed' event arrives within 5s, enable send button anyway
        const subscriptionTimeout = setTimeout(() => {
          if ($("btn-send").disabled) {
            console.warn("[Session] No 'subscribed' event received, enabling send button anyway");
            $("btn-send").disabled   = false;
          }
        }, 5000);
        // Store timeout ID so it can be cleared when subscription succeeds
        Sessions._subscriptionTimeout = subscriptionTimeout;
        Sessions.renderList();
        $("user-input").focus();
...
WS.onEvent(ev => {
  switch (ev.type) {
    ...
    case "subscribed": {
      ...
      if (Sessions._subscriptionTimeout) {
        clearTimeout(Sessions._subscriptionTimeout);
        Sessions._subscriptionTimeout = null;
      }
