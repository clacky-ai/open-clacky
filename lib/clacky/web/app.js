...
const ClackyUiPolicy = (() => {
  const raw = window.CLACKY_UI_POLICY || {};
  const bool = (value, fallback = true) => value === undefined ? fallback : !!value;
  const viewKey = (name) => name === "session" ? "chat" : name;

  function viewEnabled(name) {
    return bool(raw?.views?.[viewKey(name)], true);
  }

  function actionEnabled(name) {
    return bool(raw?.actions?.[name], true);
  }

  function chatEnabled(name) {
    return bool(raw?.chat?.[name], true);
  }

  function welcomeConfig() {
    return raw?.welcome && typeof raw.welcome === "object" ? raw.welcome : {};
  }

  function localeCode() {
    const code = raw?.locale?.code;
    return code === "zh" || code === "en" ? code : null;
  }

  function localeEditable() {
    return raw?.locale?.editable !== false;
  }

  function isHosted() {
    return !!window.NOKNO_HOSTED_MODE || Object.keys(raw).length > 0;
  }

  function fallbackTarget() {
    if (viewEnabled("welcome")) return { view: "welcome", params: {} };
    if (viewEnabled("settings")) return { view: "settings", params: {} };
...
window.ClackyUiPolicy = ClackyUiPolicy;
...
const Router = (() => {
  ...
  function _parseHash(hash) {
    ...
    if (!h)                           return _normalizeTarget("welcome", {});
    if (h === "tasks")                return _normalizeTarget("tasks", {});
    if (h === "skills")               return _normalizeTarget("skills", {});
    if (h === "channels")             return _normalizeTarget("channels", {});
    if (h === "trash")                return _normalizeTarget("trash", {});
    if (h === "profile")              return _normalizeTarget("profile", {});
    ...
    if (h === "memories")             return _normalizeTarget("profile", {});
    if (h === "settings")             return _normalizeTarget("settings", {});
    if (h === "creator")              return _normalizeTarget("creator", {});
    const m = h.match(/^session\/(.+)$/);
    if (m)                            return _normalizeTarget("session", { id: m[1] });
    return _normalizeTarget("welcome", {});
  }

  function _normalizeTarget(view, params = {}) {
    if (ClackyUiPolicy.viewEnabled(view)) return { view, params };
    return ClackyUiPolicy.fallbackTarget();
  }
  ...
  function _apply(view, params = {}) {
    const normalized = _normalizeTarget(view, params);
    if (normalized.view !== view || JSON.stringify(normalized.params) !== JSON.stringify(params)) {
      _apply(normalized.view, normalized.params);
      return;
    }
    ...
    switch (view) {
      ...
      default:  // "welcome"
        _setHash("");
        if (location.hash) setTimeout(() => _setHash(""), 0);
...
// ── Hosted welcome content ─────────────────────────────────────────────────
function _hostedWelcomePrompts() {
  const prompts = ClackyUiPolicy.welcomeConfig().prompts;
...
function _renderHostedWelcome() {
  const welcome = ClackyUiPolicy.welcomeConfig();
  const titleEl = $("welcome-title");
  const subtitleEl = $("welcome-subtitle");
  const promptsEl = $("welcome-prompts");
  const supportEl = $("welcome-support");
  ...
  if (subtitleEl) {
    const subtitle = String(welcome.subtitle || "").trim();
    if (subtitle) {
  ...
  const prompts = _hostedWelcomePrompts();
  if (promptsEl) {
    if (prompts.length) {
      promptsEl.innerHTML = prompts.map((item, index) => `
        <button class="welcome-prompt-btn" type="button" data-prompt-index="${index}">
  ...
  if (supportEl) {
    const supportText = String(welcome.support_text || "").trim();
    supportEl.textContent = supportText;
    supportEl.style.display = supportText ? "" : "none";
  }
}

function _initHostedWelcome() {
  _renderHostedWelcome();
  document.addEventListener("langchange", _renderHostedWelcome);
  const promptsEl = $("welcome-prompts");
  if (!promptsEl) return;
  promptsEl.addEventListener("click", (e) => {
    const btn = e.target.closest(".welcome-prompt-btn");
    if (!btn) return;
    const item = _hostedWelcomePrompts()[Number(btn.dataset.promptIndex)];
    if (!item) return;
...
window.bootAfterBrand = async function() {
  ...
  if (!needsOnboard) {
    ...
    WS.connect();
    if (ClackyUiPolicy.viewEnabled("tasks")) Tasks.load();
    if (ClackyUiPolicy.viewEnabled("skills")) Skills.load();
  }
};
