...
const Tasks = (() => {
  ...
  function _showPanel() {
    ...
    // Wire up the "Create Task" button
  ...
  return {
    ...
    renderSection() {
      ...
      const panelVisible = $("task-detail-panel").style.display === "flex";
      el.className = "task-item task-item-summary" + (panelVisible ? " active" : "");
      el.innerHTML = `
        <div class="task-row">
          <span class="task-icon">⏰</span>
          <div class="task-info">
      ...
      el.addEventListener("click", () => {
        // Use actual DOM visibility as the source of truth — _panelOpen can be
        // stale if a session was selected while the panel was open.
        const visible = $("task-detail-panel").style.display === "flex";
        if (visible) {
          _hidePanel();
        } else {
          _showPanel();
    ...
    async run(name) {
      ...
      if (data.session) {
        ...
        Sessions.renderList();
        // Mark this session as having a pending task to run.
        // app.js will send "run_task" over WS once "subscribed" is confirmed,
        // guaranteeing the client is ready to receive broadcasts before the
    ...
    async createInSession() {
      ...
      // Auto-send /create-task so the agent knows what to do
