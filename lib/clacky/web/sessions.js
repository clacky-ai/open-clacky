...
const Sessions = (() => {
  ...
  function _welcomeConfig() {
    const raw = _policy()?.welcomeConfig ? _policy().welcomeConfig() : window.CLACKY_UI_POLICY?.welcome;
    return raw && typeof raw === "object" ? raw : {};
  }

  function _welcomePrompts() {
    const prompts = _welcomeConfig().prompts;
  ...
  function _buildEmptyHintHtml() {
    const welcome = _welcomeConfig();
    const title    = String(welcome.title || "").trim() || I18n.t("chat.empty.title");
    const subtitle = String(welcome.subtitle || "").trim() || I18n.t("chat.empty.subtitle");
    const tip2     = I18n.t("chat.empty.tip2");
    const tip3     = I18n.t("chat.empty.tip3");
    const prompts  = _welcomePrompts();
    const tips = [];
    if (_policy()?.chatEnabled("show_skill_autocomplete")) tips.push(I18n.t("chat.empty.tip1"));
    tips.push(tip2, tip3);
    return `
      ...
      ${prompts.length ? `
        <div class="chat-empty-prompts">
          ${prompts.map((item, index) => `
            <button class="chat-empty-prompt-btn" type="button" data-prompt-index="${index}">
  ...
  function _initEmptyHint() {
    ...
    messages.addEventListener("click", (e) => {
      const btn = e.target.closest(".chat-empty-prompt-btn");
      if (!btn) return;
      const item = _welcomePrompts()[Number(btn.dataset.promptIndex)];
      if (!item) return;
      Sessions.create("general", { name: item.label, initialMessage: item.prompt });
    });
  ...
  function _initNewSessionControls() {
    const showAdvanced = _policy()?.actionEnabled("manage_session_model") !== false ||
      _policy()?.actionEnabled("manage_working_dir") !== false;
    if (!showAdvanced) {
      const arrowBtn = document.getElementById("btn-new-session-arrow");
  ...
  function _renderHistoryEvent(ev, container, historyCtx) {
    ...
    switch (ev.type) {
      ...
      case "tool_call": {
        if (_policy()?.chatEnabled("show_tool_trace") === false) break;
      ...
      case "token_usage": {
        if (_policy()?.chatEnabled("show_token_usage") === false) break;
  ...
  return {
    ...
    updateInfoBar(s) {
      ...
      if (policy && !policy.chatEnabled("show_session_info_bar")) {
      ...
      if (sibDir && s.working_dir) {
        ...
        const dirEditable = policy?.actionEnabled("manage_working_dir") !== false && policy?.chatEnabled("allow_working_dir_switch") !== false;
        sibDir.title = dirEditable ? `${s.working_dir} (${I18n.t("sib.dir.tooltip")})` : s.working_dir;
        sibDir.dataset.workingDir = s.working_dir;
        sibDir.dataset.sessionId = s.id;
        sibDir.style.cursor = dirEditable ? "pointer" : "default";
      }
      ...
      if (sibModel) {
        ...
        const modelEditable = policy?.actionEnabled("manage_session_model") !== false && policy?.chatEnabled("allow_model_switch") !== false;
        sibModel.title = modelEditable ? I18n.t("sib.model.tooltip") : (s.model || "");
        sibModel.style.cursor = modelEditable ? "pointer" : "default";
      }
      ...
      const sibCostSep = document.querySelector(".sib-sep-after-tasks");
      if (sibCost) {
        ...
        const showCost = policy?.chatEnabled("show_cost") !== false;
        sibCost.style.display = showCost ? "" : "none";
        if (sibCostSep) sibCostSep.style.display = showCost ? "" : "none";
      }
    ...
    appendToolCall(name, args, summary) {
      if (_policy()?.chatEnabled("show_tool_trace") === false) return;
      const messages = $("messages");
    ...
    appendTokenUsage(ev, container) {
      if (_policy()?.chatEnabled("show_token_usage") === false) return;
    ...
    _formatTokenSuffix(metadata) {
      if (_policy()?.chatEnabled("show_token_usage") === false) return null;
    ...
    async create(agentProfile = "general", options = {}) {
      ...
      const name = customName || "Session " + (maxN + 1);
      ...
      Sessions.renderList();
      if (initialMessage) Sessions.setPendingMessage(session.id, initialMessage);
    ...
    openNewSessionModal() {
      ...
      if (!modal) return;
      const policy = _policy();
      const modelField = $("new-session-model")?.closest(".modal-field");
      const dirField = $("new-session-directory")?.closest(".modal-field");
      if (modelField) modelField.style.display = policy?.actionEnabled("manage_session_model") ? "" : "none";
      if (dirField) dirField.style.display = policy?.actionEnabled("manage_working_dir") ? "" : "none";

      // Populate model dropdown from configured models
      if (policy?.actionEnabled("manage_session_model") !== false) _populateModelDropdown();

      // Set default working directory
      const dirInput = $("new-session-directory");
      if (dirInput && policy?.actionEnabled("manage_working_dir") !== false && !dirInput.value) {
        dirInput.value = "~/clacky_workspace";
      } else if (dirInput && policy?.actionEnabled("manage_working_dir") === false) {
        dirInput.value = "";
      }
    ...
    async createFromModal() {
      ...
      try {
        ...
        // Add optional fields
        if (policy?.actionEnabled("manage_working_dir") !== false && workingDir) payload.working_dir = workingDir;
        if (policy?.actionEnabled("manage_session_model") !== false && selectedModelId) payload.model_id = selectedModelId;
...
(function() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (modelEl) {
      if (window.ClackyUiPolicy?.actionEnabled("manage_session_model") === false ||
          window.ClackyUiPolicy?.chatEnabled("allow_model_switch") === false) {
        return;
      }
      e.stopPropagation();
...
(function() {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (dirEl) {
      if (window.ClackyUiPolicy?.actionEnabled("manage_working_dir") === false ||
          window.ClackyUiPolicy?.chatEnabled("allow_working_dir_switch") === false) {
        return;
      }
      e.stopPropagation();
