  ...
  return {
    ...
    renderList() {
      ...
      [...manual, ...scheduled].forEach(s => {
        ...
        // Double-click to rename: replace name span with an input field
    ...
    _startRename(sessionId, nameDiv, currentName) {
      ...
      // Clear current content and insert an input
      nameDiv.innerHTML = dotHtml;
      ...
      const commit = async () => {
        ...
        // Restore original name in DOM first (server response / WS will update it)
      ...
      // Stop click from triggering session select while editing
