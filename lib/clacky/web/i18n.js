...
const I18n = (() => {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      "chat.done.noCost":    "Done — {{n}} iteration(s)",
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  const _policyLocale = window.CLACKY_UI_POLICY?.locale || {};
  const _fixedPolicyLang = TRANSLATIONS[_policyLocale.code] ? _policyLocale.code : null;
  let _lang = _fixedPolicyLang || localStorage.getItem(STORAGE_KEY) || DEFAULT_LANG;
  if (_fixedPolicyLang) localStorage.setItem(STORAGE_KEY, _lang);
  ...
  function setLang(code) {
    if (_fixedPolicyLang && _policyLocale.editable === false) {
      _lang = _fixedPolicyLang;
    } else {
