...
const Settings = (() => {
  ...
  function open() {
    ...
    if (policy?.isHosted()) {
      _applyHostedPolicy();
      if (policy.actionEnabled("edit_models")) _load();
      if (policy.actionEnabled("manage_brand")) _loadBrand();
      if (policy.actionEnabled("manage_browser")) _loadBrowserStatus();
      return;
    }
    _load();
    _loadBrand();
  ...
  function _applyHostedPolicy() {
    const policy = _policy();
    if (!policy) return;

    [
      ["models-section", policy.actionEnabled("edit_models")],
      ["personalize-section", policy.actionEnabled("manage_profile")],
      ["browser-section", policy.actionEnabled("manage_browser")],
      ["brand-license-section", policy.actionEnabled("manage_brand")],
    ].forEach(([id, visible]) => {
      const section = document.getElementById(id);
      if (section) section.style.display = visible ? "" : "none";
    });

    _renderHostedReadonlySettings();
  }

  async function _renderHostedReadonlySettings() {
    const policy = _policy();
    const section = document.getElementById("language-section");
    const buttonRow = section?.querySelector(".settings-lang-btns");
    if (!policy || !section || !buttonRow) return;

    let readonly = document.getElementById("settings-hosted-readonly");
    if (!readonly) {
      readonly = document.createElement("div");
      readonly.id = "settings-hosted-readonly";
      readonly.className = "brand-status-fields";
      buttonRow.insertAdjacentElement("afterend", readonly);
    }

    buttonRow.style.display = policy.localeEditable() ? "" : "none";

    const version = await _loadCurrentVersion();
    const localeCode = policy.localeCode() || I18n.lang();
    const localeLabel = localeCode === "zh" ? "中文" : "English";
    const versionLabel = I18n.lang() === "zh" ? "版本" : "Version";

    readonly.innerHTML = `
      <div class="brand-status-row">
        <span class="brand-status-label">${_esc(I18n.t("settings.lang.title"))}</span>
        <span class="brand-status-value">${_esc(localeLabel)}</span>
      </div>
      <div class="brand-status-row">
        <span class="brand-status-label">${_esc(versionLabel)}</span>
        <span class="brand-status-value">${_esc(version || "—")}</span>
      </div>
    `;
  ...
  async function _loadCurrentVersion() {
    if (_hostedVersion !== null) return _hostedVersion;
    try {
      const res = await fetch("/api/version");
      const data = await res.json();
      _hostedVersion = data.current ? `v${data.current}` : "";
    } catch (_) {
      _hostedVersion = "";
    }
    return _hostedVersion;
  }
  ...
  async function _loadBrand() {
    if (_policy()?.isHosted() && !_policy()?.actionEnabled("manage_brand")) {
