...
const Skills = (() => {
  ...
  return {
    ...
    onPanelShow() {
      ...
      if (window.ClackyUiPolicy?.isHosted()) return;
...
const SkillAC = (() => {
  ...
  function _skillUiEnabled() {
    return window.ClackyUiPolicy?.chatEnabled("show_skill_autocomplete") !== false;
  }

  function _slashButtonEnabled() {
    return window.ClackyUiPolicy?.chatEnabled("show_slash_button") !== false;
  }
  ...
  function _initDOMBindings() {
    if (!_slashButtonEnabled()) {
      $("btn-slash").style.display = "none";
    }
    if (!_skillUiEnabled()) {
      $("skill-autocomplete").style.display = "none";
    }
