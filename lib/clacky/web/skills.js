...
const Skills = (() => {
  ...
  return {
    ...
    /** Install a skill from the Public Store by calling the install API directly.
     *  Shows an inline loading state on the button and refreshes the skills list on success.
    ...
    async installFromStore(slug, title, downloadUrl, btn) {
      ...
      try {
        const res  = await fetch(`/api/store/skills/${encodeURIComponent(slug)}/install`, {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (!res.ok || !data.ok) {
          alert(`Failed to install "${title}": ` + (data.error || "unknown error"));
          if (btn) {
            btn.disabled = false;
        ...
        // Reload skills list — the new skill will show as "✓ Installed"
        await Skills.load();
      } catch (e) {
        console.error("[Skills] installFromStore failed", e);
        alert(`Network error while installing "${title}".`);
        if (btn) {
          btn.disabled = false;
          btn.textContent = "Install";
        }
